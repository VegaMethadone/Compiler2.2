...
void parseTree(Node *node, Block *block, Block *backdoor) {
    ...
    if (strcmp(node->type, "listStatement") == 0) {
        ...
        if (findCall(node->right)) {
            ...
            // тут надо обработаться еще return
            if (strcmp(node->left->value, "return") == 0)
...
void createDotStreamFile(Node *root, char *filename) {
    ...
    //bool addedEnd = false;
    //addEndNode(root, &addedEnd);
    //addEndNode(root);


    //
    //parseTree(root->right, graph->start, NULL);
    ...
    for (int  i = 0; i < size;  i++) {
        ...
        //freeStreamGraph(current);
    }
     //generateDotStream(graph->start, file, &array);

    fprintf(file, "}\n");

    fclose(file);

    //freeStreamGraph(graph);
    //freeArray(&array);
}
